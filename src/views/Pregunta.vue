   ...
   <div class="pregunta">
        <div class="card">
         <div class="card-header bg-info text-white">
            <h4>Preguntas
              <a data-toggle="modal" data-target="#registroPreguntasModal" class="registrarMas float-right pr-3">+</a>
            </h4>
          </div>
          <div class="card-body">
            <div class="acordeon">
              <div class="acordeon-item" v-for="(pregunta, preguntaIndex) in repositorioPreguntas" :key="pregunta.timestamp">
                  <input type="checkbox" name="acordeon-p"
                    v-bind="{id: 'acordeon-p-item-' + preguntaIndex}" 
                  >
                  <label 
                    v-bind="{for: 'acordeon-p-item-' + preguntaIndex}" 
                    class="acordeon-titulo">
                    <h6>{{ pregunta.descripcion }}</h6>
                  </label>
                  <div class="acordeon-contenido" >
                    <div class="">
                      <p>Pregunta: {{ pregunta.descripcion }}</p>
                    </div>
                  </div>
              </div>
            </div>
...
import registrarPreguntas from '@/views/RegistrarPreguntas.vue'
...
export default {
  ...
  methods: {
    getPreguntas(){
      ...
      // this.repositorioPreguntas = querySnapshot;
          querySnapshot.forEach((doc) => {
              this.repositorioPreguntas.push(doc);
          });
      });
    },
     registrar: function() {
